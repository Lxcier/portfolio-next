...
const MagicButton = ({
  ...
  position?: string;
  handleClick?: () => void;
  className?: string; // Nome de classes adicionais para adicionar ao botão;
  otherClasses?: string;
}) => {
  return (
    <button
      className={`relative inline-flex h-12 w-fit overflow-hidden rounded-lg p-[1px] focus:outline-none ${className}`}
      onClick={handleClick}
    >
